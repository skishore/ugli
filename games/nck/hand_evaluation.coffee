...
pow_base = 100
pows = (Math.pow(pow_base,ix) for ix in [0...5])
...
get_tiebreak = (ranks, num) ->
  if not num?
    num = 5
  ranks = (parseInt(rank) for rank of ranks)
  ranks.sort((x,y)->return x-y)
  tiebreak = 0
  for i in [-1...-num-1]
    tiebreak *= pow_base
    tiebreak += parseInt ranks[ranks.length + i]
  return tiebreak
...
classify_hand = (hand) ->
  ...
  if dict_len(rank_count_sets[4]) > 0
    return [KIND4, Math.max.apply(null, rank_count_sets[4])]

  # CHECK FOR FULL HOUSE

  if (dict_len(rank_count_sets[3]) >= 2) or (dict_len(rank_count_sets[3]) == 1 and dict_len(rank_count_sets[2]) >= 1)
  ...
  for rank in ranks
    rank_set[rank] = true
  high_rank = get_straight(rank_set)
  if high_rank > 0
    return [STRAIGHT, high_rank]

  # CHECK FOR THREE OF A KIND

  if dict_len(rank_count_sets[3]) >= 1
    assert dict_len(rank_count_sets[2]) == 0 and dict_len(rank_count_sets[3]) == 1 and dict_len(rank_count_sets[4]) == 0
    return [KIND3, get_tiebreak(rank_count_sets[1], 2)]

  # CHECK FOR TWO-PAIR

  if dict_len(rank_count_sets[2]) >= 2
    assert dict_len(rank_count_sets[3]) == 0 and dict_len(rank_count_sets[4]) == 0
    sorted_pairs = (parseInt i for i of rank_count_sets[2]).sort((x,y)->return x-y)
    last_card = -1

    for x in sorted_pairs.slice(0,-2)
      if x > last_card
        last_card = x
    for x of rank_count_sets[1]
      if x > last_card
        last_card = x
    assert last_card != -1, 'Last card was -1?'

    tiebreak = sorted_pairs[sorted_pairs.length-1] * pows[2] + sorted_pairs[sorted_pairs.length-2] * pows[1] + last_card
    return [TWO_PAIR, tiebreak]

  # CHECK FOR PAIR

  if dict_len(rank_count_sets[2]) >= 1
    assert dict_len(rank_count_sets[2]) == 1 and dict_len(rank_count_sets[3]) == 0 and dict_len(rank_count_sets[4]) == 0
    pair = -1
    for x of rank_count_sets[2]
      if x > pair
        pair = x
    assert pair != -1, 'Pair was -1?'
    tiebreak = pair * pows[3] + get_tiebreak(rank_count_sets[1], 3)
    return [PAIR, tiebreak]

  # CHECK FOR HIGH
  assert dict_len(rank_count_sets[2]) == 0 and dict_len(rank_count_sets[3]) == 0 and dict_len(rank_count_sets[4]) == 0
...
compare_hands = (handA, handB)->
  newhandA = (string_to_card(x) for x in handA)
  newhandB = (string_to_card(x) for x in handB)
  ret = poker_hand_comparator(newhandA, newhandB)
  return ret
